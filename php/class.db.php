<?php
    ...
    class Validate {
        public static function userId(string $userId) {
            return strlen($userId) > 0 && preg_match("/^[0-9]{1,5}$/", $userId);
        }

        public static function userName(string $username) {
            return strlen($username) > 0 && preg_match("/^[a-zA-Z0-9_]{1,20}$/", $username);
        }

        public static function userPassword(string $password) {
            return strlen($password) > 0 && preg_match("/^(?=.*?[a-z])(?=.*?[0-9]).{8,20}$/", $password);
        }

        public static function userToken(string $token) {
            return strlen($token) > 0 && preg_match("/^[a-zA-Z0-9]{30,40}$/", $token);
        }

        public static function chatName(string $name) {
            return strlen($name) > 0 && preg_match("/^[a-zA-Z0-9_]{1,20}$/", $name);
        }

        public static function chatId(string $chatId) {
            return strlen($chatId) > 0 && preg_match("/^[0-9]{1,5}$/", $chatId);
        }
    }
    ...
    class DB {
        ...
        public function createUser(string $username, string $password) {
            ...
            return array("user_id" => $row[0], "token" => $this->createToken($row[0], $password));
        }

        public function loginUser(string $username, string $password) {
            $res = mysqli_query($this->conn, "SELECT creds.salt, users.id FROM users JOIN creds ON creds.user_id=users.id WHERE users.name='" . $username . "' LIMIT 1");
                
            if($res->num_rows <= 0)
                return false;
        
            $row = mysqli_fetch_row($res);
            $hash = hash("sha256", $password . $row[0]);
            $userId = $row[1];

            $res = mysqli_query($this->conn, "SELECT user_id FROM creds WHERE password='" . $hash . "' AND user_id='" . $userId . "' LIMIT 1");
            ...
            return array("user_id" => $row[0], "token" => $this->createToken($userId, $password));
        }
        ...
        public function joinChat(string $chatId, string $userId, string $token) {
            $sql = "INSERT INTO chat_relations(user_id, chat_id) SELECT '" . $userId . "', '" . $chatId . "' WHERE (SELECT user_id FROM tokens WHERE token='" . $token . "' AND user_id='" . $userId . "')='" . $userId . "'";
            $res = mysqli_query($this->conn, $sql);

            return $res && mysqli_affected_rows($this->conn) > 0;
        }
